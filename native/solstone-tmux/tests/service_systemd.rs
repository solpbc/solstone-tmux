...
use solstone_tmux::service::systemd::{UNIT_NAME, artifact_path, render};
use solstone_tmux::service::{COMMAND_TIMEOUT, LocalObserver, STATE_FILENAME, ServiceController};
use support::{
...
fn systemd_unit_matches_required_sections() {
    let bytes = render(
        Path::new("/opt/solstone/bin/solstone-tmux"),
...
ExecStart=\"/opt/solstone/bin/solstone-tmux\" run\n\
Restart=on-failure\nRestartSec=5\n"
    ));
...
fn systemd_escaping_handles_spaces_and_metacharacters() {
    let bytes = render(
        Path::new("/owner/a b/$bin;100%/solstone-tmux"),
        OsStr::new("/owner/a b/$PATH;100%:/usr/bin"),
    )
    .expect("render escaped unit");
    let unit = String::from_utf8(bytes).expect("UTF-8 unit");
    assert!(unit.contains("ExecStart=\"/owner/a b/$bin;100%%/solstone-tmux\" run"));
...
fn unjoinable_service_path_is_actionable_and_does_not_drop_tmux_directory() {
    ...
    let binary = root.join("bin/solstone-tmux");
...
fn systemd_replacement_stops_before_atomic_write() {
    ...
    fs::write(
        &artifact,
        render(Path::new("/old/solstone-tmux"), OsStr::new("/old")).expect("old unit"),
    )
...
impl ServiceFixture {
    ...
    fn controller<'a>(
        &'a self,
        runner: &'a dyn solstone_tmux::command::CommandRunner,
    ) -> Controller<'a> {
        Controller(
...
impl solstone_tmux::command::CommandRunner for OrderingRunner {
    fn run<'a>(
        &'a self,
        invocation: CommandInvocation,
    ) -> std::pin::Pin<
        Box<
            dyn std::future::Future<
                    Output = Result<
                        solstone_tmux::command::CommandOutput,
                        solstone_tmux::command::CommandError,
                    >,
                > + Send
                + 'a,
        >,
    > {
        Box::pin(async move {
            ...
            Ok(solstone_tmux::command::CommandOutput {
