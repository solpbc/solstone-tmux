...
#[derive(Clone, Debug, Deserialize, Eq, PartialEq, Serialize)]
#[serde(deny_unknown_fields)]
pub struct ClientsSelfPutResponse {
    pub protocol_version: u32,
    pub revision: u64,
    #[serde(default)]
    pub reported: Option<ClientsSelfReportedSnapshot>,
    ...
    F: Fn() -> Option<String>,
{
    ...
    if let Some(ref version) = get_response.journal.version
        && !version.is_empty()
    {
        version_refresh.apply_validated_version(version);
    }
    ...
    if put_status == StatusCode::CONFLICT {
        ...
        if let Some(ref version) = retry_get_response.journal.version
            && !version.is_empty()
        {
            version_refresh.apply_validated_version(version);
        }
...
mod tests {
    ...
    fn decode_get_response_validates_protocol_version() {
        ...
        assert_eq!(decoded.journal.version, Some("2026.9.1".to_owned()));
