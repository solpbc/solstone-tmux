...
use solstone_tmux::command::TokioCommandRunner;
use solstone_tmux::config::{RuntimeConfig, system_hostname};
...
fn run_native(
    platform: solstone_tmux::paths::PlatformKind,
        ...
        .map_err(|error| error.to_string())?
    {
        if matches!(
            ...
            eprintln!(
                "solstone-tmux: warning: recovery {:?} for {}: {}",
                record.action,
            ...
            event = async {
                loop {
                    ...
                    if observer_shutdown.changed().await.is_err() {
                        break solstone_tmux::observer::ShutdownEvent::Injected;
                    }
                }
            } => event,
        }
    ...
    for failure in &exit.failures {
        eprintln!("solstone-tmux: {failure}");
    }
    Ok(exit.exit_code)
}
